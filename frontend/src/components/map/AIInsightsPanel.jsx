...
export default function AIInsightsPanel({ activeFilter, intelligence }) {
  ...
  return <div style={{ display:'flex',flexDirection:'column',minHeight:'100%',background:'var(--bg-primary)',borderLeft:'1px solid var(--border)',borderRadius:'0 16px 16px 0',overflow:'hidden' }}>
    ...
    <div key={tab} style={{padding:'0 20px 20px',animation:'djagaInsightPanelIn 320ms cubic-bezier(.2,.8,.2,1) both'}}>
      ...
      {tab === 2 && <><p style={{fontSize:'11px',lineHeight:1.55,color:'var(--text-tertiary)',margin:'2px 0 16px'}}>Live intelligence summary across DJAGA’s monitored Malaysian scam signals.</p><MonthlyTrend points={intelligence.monthly_trend}/>{[['Reports today',stats.totalReportsToday || 0,'var(--accent)'],['Active alerts',stats.activeAlerts || 0,'var(--threat)'],['New today',`+${stats.newSinceYesterday || 0}`,'var(--warning)'],['AI scans today',stats.aiScansToday || 0,'var(--safe)'],['Most affected area',stats.mostAffectedCity || '—','var(--text-primary)']].map(([label,value,color])=><div key={label} style={{padding:'15px 0',borderBottom:'1px solid var(--border)',display:'flex',justifyContent:'space-between',alignItems:'center',gap:'10px'}}><span style={{fontSize:'12px',color:'var(--text-secondary)'}}>{label}</span><strong style={{fontSize:'17px',color}}><CountUp value={value}/></strong></div>)}<CityRankingList activeFilter={activeFilter} cityStats={intelligence.city_stats} scamTypes={intelligence.scam_types} mapPoints={intelligence.map_points}/></>}
    </div>
  </div>;
}
